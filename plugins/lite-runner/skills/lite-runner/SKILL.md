...
2. `python -c "import worldjen; …"` using the **same** `python` they use for `run.py` / the wrapped command.
...
def main() -> None:
    try:
        ...
        print("Install the published SDK in this env: pip install worldjen", file=sys.stderr)
        sys.exit(1)
    ...
    upload_to_presigned(upload_info["uploadUrl"], video, content_type=_content_type(video))
    ...
    while waited < args.max_wait:
        ...
        scores = videos[0].get("scores") if isinstance(videos, list) and videos else None
